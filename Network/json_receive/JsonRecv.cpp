...
int main(int argc, char * argv[])
{
	...
	ShutdownSockets();*/
	/*
	This is a modified version of the gafferon games program. This program only receives packets and prompts
	users for a specific port to open.
	Source: "Networking for Game Programmers" - http://www.gaffer.org/networking-for-game-programmers
	*/
	...
	while (bytes_read == 0){
		...
		if (socket.Send(sender, buffer, sizeof(buffer)))
		...
		Coordinates coord;
		coord.latitude = documentC["latitude"].GetDouble();
		coord.longitude = documentC["longitude"].GetDouble();

		std::cout << "longitude value is: " << coord.longitude << std::endl;
		std::cout << "latitude value is : " << coord.latitude << std::endl;
